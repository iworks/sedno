...
class Sedno_Cookie_Notice extends Sedno {
	/**
	 * Module option name.
	 *
	 * @var string
	...
	private function set_data() {
		...
		$this->data = array(
			...
			'text'    => sprintf(
				__( 'We use cookies and similar technologies to provide services and to gather information for statistical and other purposes. You can change the way you want the cookies to be stored or accessed on your device in the settings of your browser. If you do not agree, change the settings of your browser. For more information, refer to our %s.', 'sedno' ),
				sprintf(
					'<a href="%s">%s</a>',
					get_privacy_policy_url(),
					_x( 'Privacy Policy', 'in cookie message, mayby propoer form', 'sedno' )
				)
			),
		);
	}
	...
	public function add_cookie_notice() {
		...
		$content .= sprintf(
			'<div class="cookie-notice-container"><span id="sedno-cn-notice-text" class="text">%s</span>',
			$this->data['text']
		);
		// Data.
		$content .= sprintf(
			'<span><a href="#" class="button sedno-cn-set-cookie" aria-label="%s">%s</a></span>',
			esc_attr__( 'Close cookie information.', 'sedno' ),
			esc_html__( 'Close cookie information', 'sedno' )
		);
		$content .= '</div>';
		$content .= '</div>';
		$content .= PHP_EOL;
